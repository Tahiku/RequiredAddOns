...
-- Make sure we are properly scaled.
    ...
    local title = " " -- local title = "Addon Control Panel"
    local version = GetAddOnMetadata(ACP_ADDON_NAME, "Version") -- local version = GetAddOnMetadata(ACP_ADDON_NAME, "Version")
    if version then
...
function ACP:SaveSet(set)
    ...
    for i=1,GetNumAddOns() do
        local name, title, notes, enabled, isondemand, reason, security = GetAddOnInfo(i)
